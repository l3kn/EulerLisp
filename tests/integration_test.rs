...
fn definitions() {
    ...
    assert_eq!(ev.eval_str("b", 0), Ok(Number(2)));
}
...
fn builtin_read() {
    let mut ev = Evaluator::new();
    assert_eq!(ev.eval_str("(read \"1\")", 0), Ok(Number(1)));
}

#[test]
fn builtin_eval() {
    let mut ev = Evaluator::new();
    assert_eq!(ev.eval_str("(eval '(+ 1 2))", 0), Ok(Number(3)));
    assert_eq!(ev.eval_str("(eval (read \"(+ 1 2)\"))", 0), Ok(Number(3)));
}
...
fn recursion_test() {
    ...
    assert_eq!(ev.eval_str("(fac 3)", 0), Ok(Number(6)))
}
...
fn test_delay_force_env() {
    ...
    assert_eq!(ev.eval_str("(force (wrap 1))", 0), Ok(Number(1)));
}
...
fn test_variadic_fn() {
    ...
    assert_eq!(ev.eval_str("(arg-len 1 2 3)", 0), Ok(Number(3)));

    ev.eval_str("(defn arg-fst (a . as) a)", 0);
    assert_eq!(ev.eval_str("(arg-fst 1 2 3)", 0), Ok(Number(1)));
